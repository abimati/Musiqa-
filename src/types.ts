...
export interface SongNote extends RawNote {
  ...
  lane: number; // 0, 1, 2, 3
  isSpawned?: boolean;
  hitState?: 'none' | 'hit' | 'miss' | 'holding' | 'released_early';
  holdProgress?: number; // duration completed
  visualDuration?: number; // interlocking chain visual duration
  releasedAtTime?: number; // tracking early release for continuous scrolling
  touchY?: number; // point of initial touch
  releaseYStart?: number; // visual Y where early release started
}
