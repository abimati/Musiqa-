...
export function GameCanvas({ song, onGameOver, onVictory, onQuit }: GameCanvasProps) {
  ...
  // Local game-over / victory report overlays
  const [localGameOverReport, setLocalGameOverReport] = useState<ScoreReport | null>(null);
  const [localIsVictory, setLocalIsVictory] = useState(false);
  const [isGameOverSequence, setIsGameOverSequence] = useState(false);
   ...
   */
  const triggerHitTap = (lane: number, touchY?: number) => {
    ...
    if (candidate) {
      ...
      if (isValidTouch) {
        if (candidate.isHold) {
          ...
          candidate.touchY = touchY;
          registerHit(candidate, absDiff, lane, touchY);
        } else {
  ...
  const registerHit = (note: SongNote, diff: number, lane: number, touchY?: number) => {
    ...
    let hitY: number | undefined = touchY;
    if (hitY === undefined) {
      ...
      if (canvas) {
        ...
        const pxPerSecond = hitZoneY / 1.25; // 1.25 is fallDuration
        const audioTime = globalAudioSynth.getElapsedTime();
        // Calculate current visual bottom coordinate of this note at the exact moment of play
        const visualY = hitZoneY - (note.time - audioTime) * pxPerSecond;
        hitY = Math.max(10, Math.min(height * 0.95, visualY));
      }
    }

    // Spawn rating float text & particles
    triggerFloatText(ratingText, lane, ratingColor);
    spawnParticles(lane, ratingColor, hitY);
  ...
  const registerMiss = (note: SongNote | null, lane: number, reason: string) => {
    ...
    let missY: number | undefined = undefined;
    const canvas = canvasRef.current;
    if (note && canvas) {
      const dpr = window.devicePixelRatio || 1;
      const height = canvas.height / dpr;
      const hitZoneY = height * 0.70;
      const pxPerSecond = hitZoneY / 1.25; // 1.25 is fallDuration
      const audioTime = globalAudioSynth.getElapsedTime();
      const visualY = hitZoneY - (note.time - audioTime) * pxPerSecond;
      missY = Math.max(10, Math.min(height * 0.95, visualY));
    }

    spawnParticles(lane, "#f43f5e", missY);
  ...
  const triggerFailure = () => {
    ...
    setTimeout(() => {
      ...
      setLocalIsVictory(false);
    }, 800);
  };

  // Canvas particle emitters
  const spawnParticles = (lane: number, color: string, y?: number) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const startY = y !== undefined ? y : height * 0.85; // hit row position
  ...
  const localRestart = () => {
    // 1. Reset report state so popup disappears
    setLocalGameOverReport(null);
    setIsGameOverSequence(false);

    // 2. Clear particles & floating text
    particlesRef.current = [];
    textFeedbackRef.current = [];

    // 3. Re-parse and regenerate notes to fresh state
    const baseNotes = song.notes || [];
    const completedNotes = generateFullChart(baseNotes, song.totalNotes, song.tempoBPM);
    ...
    activeNotesRef.current.forEach(note => {
      ...
      if (note.hitState === 'holding') {
        ...
        if (correspondingPressed) {
          ...
          if (Math.random() < 0.3) {
            let visualHoldingY: number | undefined = note.touchY;
            if (visualHoldingY === undefined) {
              const canvas = canvasRef.current;
              if (canvas) {
                const dpr = window.devicePixelRatio || 1;
                const height = canvas.height / dpr;
                const hitZoneY = height * 0.70; // aligned with drawUniverse judgment line
                const pxPerSecond = hitZoneY / 1.25;
                visualHoldingY = hitZoneY - (note.time - audioTime) * pxPerSecond;
                if (audioTime >= note.time) {
                  visualHoldingY = hitZoneY;
                }
                visualHoldingY = Math.max(10, Math.min(height * 0.95, visualHoldingY));
              }
            }
            spawnParticles(note.lane, "#22d3ee", visualHoldingY);
          }
          ...
          note.hitState = 'released_early';
          note.releasedAtTime = audioTime;
          
          const canvas = canvasRef.current;
          let calculatedHitZoneY = 500; // safe fallback height * 0.70
          if (canvas) {
            const dpr = window.devicePixelRatio || 1;
            calculatedHitZoneY = (canvas.height / dpr) * 0.70;
          }
          note.releaseYStart = note.touchY !== undefined ? note.touchY : calculatedHitZoneY;
  ...
  const drawUniverse = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, audioTime: number) => {
    ...
    activeNotesRef.current.forEach(note => {
      ...
      let yBottom = hitZoneY - (noteStartTime - audioTime) * pxPerSecond;
      const yTop = hitZoneY - (noteEndTime - audioTime) * pxPerSecond;

      if (note.hitState === 'holding') {
        // Snap/shrink bottom slice directly to the point where the player is touching it
        yBottom = note.touchY !== undefined ? note.touchY : hitZoneY;
      } else if (note.hitState === 'released_early') {
        // Continue falling down from the visual touch position at release
        const releaseY = note.releaseYStart !== undefined ? note.releaseYStart : hitZoneY;
        const elapsedSinceRelease = audioTime - (note.releasedAtTime || 0);
        yBottom = releaseY + elapsedSinceRelease * pxPerSecond;
      }

      const computedHeight = Math.max(0, yBottom - yTop);
      ...
      if (!note.isHold) {
        ...
        if (note.hitState === 'released_early') {
          const elapsed = audioTime - (note.releasedAtTime || 0);
          if (elapsed > 0.4) {
            note.hitState = 'hit'; // finish rendering
            ctx.restore();
            return;
          }
          ctx.globalAlpha = Math.max(0, 1 - elapsed / 0.4);
        }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-10 flex flex-col justify-between p-4 md:p-6">
        ...
        {/* Lives Indicator (Centered at absolute top center) */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 pointer-events-auto flex justify-center items-center">
          <div className="bg-black/30 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg">
            <span className="font-mono flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <span 
                  key={i} 
                  className={`text-2xl transition-all duration-300 select-none ${
                    i < lives ? "text-rose-500 scale-125 font-bold filter drop-shadow-[0_0_8px_rgba(244,63,94,1)]" : "text-white/10 scale-90"
                  }`}
                >
                  ♥
                </span>
        ...
        <div className="flex justify-between items-start w-full">
          {/* Left-Top: Empty spacer to balance layout */}
        ...
        <div className="absolute top-18 left-1/2 -translate-x-1/2 flex flex-col items-center w-3/4 max-w-sm">
          {/* Top Progression Bar */}
      ...
      {!isPlaying && localGameOverReport === null && !isGameOverSequence && (
        <div className="absolute inset-0 bg-[#06020c]/90 backdrop-blur-md flex flex-col items-center justify-center p-6 z-30 text-center select-none pointer-events-auto">
          ...
          <div className="max-w-xs relative z-10">
            ...
            <p className="text-xs font-mono text-cyan-400 mb-8 uppercase tracking-widest animate-pulse">
      ...
      {countdown !== null && (
        <div className="absolute inset-0 bg-black/25 backdrop-blur-[1.5px] flex flex-col items-center justify-center z-25 pointer-events-none">
      ...
      {/* 5. SMOOTH GAME OVER / VICTORY MODAL POPUP (Z-Index 40) */}
      {localGameOverReport !== null && (
        <GameOver
          report={localGameOverReport}
          isVictory={localIsVictory}
          onRetry={localRestart}
